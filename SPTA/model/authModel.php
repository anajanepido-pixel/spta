    ...
    function register($data)
    {
        ...
        if ($role == "Parent") {
            ...
            $sql = "INSERT INTO parent
                    (parent_fname, parent_mname, parent_lname, parent_extn, parent_email, parent_address, parent_relationship, parent_contact_no, username, password, role_P, parent_status, parent_date_created)
                    VALUES
                    (:fname, :mname, :lname, :extn, :email, :address, :relationship, :contact, :username, :password, :role_P, 'Parent', 'Active', NOW())";

            $stmt = $this->conn->prepare($sql);

            return $stmt->execute([
                'fname' => $data['parent_fname'],
                'mname' => $data['parent_mname'],
                'lname' => $data['parent_lname'],
                'extn' => $data['parent_extn'],
                'email' => $data['parent_email'],
                'address' => $data['parent_address'],
                'relationship' => $data['parent_relationship'],
                'contact' => $data['parent_contact_no'],
                'username' => $username,
                'password' => $password,
                'role_P' => $data['role_P']
            ]);
